/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package Latihan.Praktikum.NomerEmpat;

import java.util.Scanner;

/**
 *
 * @author Asus
 */
public class dataMahasiswaMain {
    public static void main(String[] args) {
        Scanner sc = new Scanner (System.in);
        
        System.out.println("__________________________________________\n");
        System.out.println("| \t\tData Mahasiswa\t\t |");
        System.out.println("__________________________________________");
        
        System.out.print("Banyak Mahasiswa\t : ");
        int n = sc.nextInt();
        dataMahasiswa [] mhsArray = new dataMahasiswa [n];
        for (int i = 0; i < mhsArray.length; i++){
            mhsArray[i] = new dataMahasiswa();
            System.out.println("Masukkan data mahasiswa ke "+(i+1));
            System.out.print("Masukkan Nama Mahasiswa\t : ");
            mhsArray[i].nama = sc.next();
            System.out.print("Masukkan NIM\t\t : ");
            mhsArray[i].NIM = sc.nextInt();
            System.out.print("Masukkan jenis kelamin\t : ");
            mhsArray[i].jenisKelamin = sc.next().charAt(0);
            System.out.print("Masukkan IPK\t\t\t : ");
            mhsArray[i].ipk = sc.nextDouble(); 
        }
        System.out.println();
        System.out.println("__________________________________________");
        for (int i = 0; i < mhsArray.length; i++){
            System.out.println("Data mahasiswa ke "+(i+1));
            System.out.println("Nama Mahasiswa\t\t : "+mhsArray[i].nama);
            System.out.println("NIM Mahasiswa\t\t : "+mhsArray[i].NIM);
            System.out.println("Jenis kelamin\t\t : "+mhsArray[i].jenisKelamin);
            System.out.println("IPK\t\t\t : "+mhsArray[i].ipk);
        }
    }
}
